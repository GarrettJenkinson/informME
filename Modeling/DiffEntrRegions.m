...
% with AnalysisForChr.m/MergeAnalysis.m for both phenotypes and bed files 
% generated with MakeBEDfilesAnalysisComp.m.
%
% Example usage: 
% DiffEntrRegions( 'coloncancer', 'colonnormal', 'Human')
%
% Required Inputs:
%
% phenoName1
%           The name of the first phenotype for which differential bed 
%           files have been made.
%
% phenoName2
%           The name of the second phenotype for which differential bed 
%           files have been made.
%
% species
...
%
function DiffEntrRegions( phenoName1, phenoName2, species  ,varargin)
...
addRequired(p,'phenoName1')
addRequired(p,'phenoName2')
...
parse(p,phenoName1, phenoName2, species,varargin{:})
...
%
bedFileName = [trackPath 'DEU-' phenoName1  '-VS-' phenoName2 '.bed'];
...
bedFileIDfiltered = fopen([trackPath 'DER-'  phenoName1 '-VS-' phenoName2 '.bed'],'w');%...
            %'_window' num2str(numRegInWindow) '_thresh' num2str(threshVal) '.bed'],'w');

hardCodedOptions = ['track  type=bedGraph visibility=full maxHeightPixels=100:15:11 autoScale=off viewLimits=-1.0:1.0 color=190,0,0 altColor=0,140,0 name=DER-' phenoName1 '-VS-' phenoName2];% ...
