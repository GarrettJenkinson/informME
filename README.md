...
Run install.sh in informME directory. During the interactive installing process the user will be asked different regarding the locations of default directories. Dependencies, such as GMP, MPFR, MPREAL, and Eigen, will be automatically installed during this process if these are not already available, and informME's C++ MEX code will be compiled.
...
NOTE: This step must be completed before proceeding with the next step, but is only done once per reference genome. 
...
where "path" is the path containing the BED files and "asy" is the assembly of the reference genome used to generate the BED files (for example, "asy" must be set to "hg19" when the Human assembly hg19 is used).

NOTE: For this utility, the following tools must be installed on $PATH: bedtools, bedClip, bedGraphToBigWig, and fetchChromSizes
