...
implements informME, as well as the BASH wrappers and example 
submission scripts used to run informME on Sun Grid Engine (SGE) 
or SLURM computer clusters. 
...
o MATLAB R2013b 64-bit and MATLAB R2016b 64-bit  with 
  Bioinformatics and Symbolic Math Toolboxes 
...
o gcc/g++ compiler - v4.4.7-4 and v4.9.2 (to compile 
  MATLAB MEX code)
...
informME/src 		   - contains all bash wrappers which include (i) help
			     file, and (ii) comprehensive toy example. Also contains
                 	     the MATLAB, C++, and R informME source codes.

informME/cluster 	   - contains templates for running informME on SGE and 
			     SLURM clusters.

informME/third_party 	   - third party MATLAB software used by informME.
...
GMP, MPFR, MPREAL, and Eigen are all C++ dependencies that
the installation script will attempt to install for you as
needed.
...
Run install.sh in informME directory. During the interactive installing process
different questions will be asked to the user in regards to default directories. 
Dependencies such as GMP, MPFR, MPREAL, and Eigen will be automatically installed 
during this process if these are not already available, and informME's C++ MEX
code will be compiled.

Note: the following environment variables will be defined in a configuration
file stored in ~/.informME/informME.config. The following environment variables
will be then accessed throughout multiple points in the informME pipeline:

o REFGENEDIR: directory where the CpGlocationChrX.mat are stored.
...
These environment variables can be overwritten through optional arguments when
running any part of informME. If these environment variables are not defined,
the user must pass the corresponding paths as optional arguments.
...
indicated order (type the name of the command with no 
arguments to see help file and instructions in each case):
...
This step analyzes the reference genome and produces a MATLAB 
...
Wrapper that takes BAM file as input and generates matrices 
for informME. Each chromosome subdirectory includes the MATLAB 
file phenoName_matrices.mat. aach file contains the following 
information for each genomic region, which will be subsequently 
...
(in base pairs) non-overlapping regions of the genome. Each 
phenoName_fit.mat file contains the following information for 
each genomic region used in model estimation: 
...
This is followed by the methylation analysis of the given 
...
relative dissipated energies. Each chromosome subdirectory 
...
results obtained after running informME_run.sh for a given
phenotype passed as argument.
...
This function makes BED files for the methylation analysis 
results obtained after running informME_run.sh for two given
phenotypes passed as arguments.
...
provided post-processing utilities.
...
[2] Jenkinson, G., Abante, J., Feinberg, A.P., and Goutsias, J. 
    (2017), An information-theoretic approach to the modeling and 
...
v0.3.0 -    Code reorganized into source and bin directory 
            structure. Wrapper shell scripts added to all
            user callable functions. I/O directories no
            longer inside code directories. General UI 
            improvements for easier usage.
...
All code authored by Garrett Jenkinson or Jordi Abante 
in informME is licensed under a GPLv3 license; exceptions 
to GPL licensing are the files contained in the following 
directories:
