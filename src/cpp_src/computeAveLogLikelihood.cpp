 ...
 * 
 * Last Modified: 9/8/17
 *
 * usage:
 * avelogLikelihood = computeAveLogLikelihood(An,Cn,dataMatrixVec,CpGstart,CpGend);
 */
    ...
    const int    *dataMatrixVec = (int *)mxGetData(prhs[2]); // read in third rhs argument
    ...
    for (int k=0;k<K;k++){
        ...
        for (int CpG = 0;CpG<s+1;CpG++){
             x_r_rPLUSs[CpG]=dataMatrixVec[obsCount];
             obsCount += 1;
        }        
