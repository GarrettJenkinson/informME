...
# Getopt command
TEMP="$(getopt -o hr:b:m:e:a:d:l:q: -l help,refdir:,bamdir:,matdir:,estdir:,analdir:,outdir:,MATLICENSE:,threads:,time_limit: -n "$script_name.sh" -- "$@")"
...
echo "[$(date)]: Processing chromosome: ${chr_num}" 
seq "$total_proc" | xargs -I {X} --max-proc "$threads" bash -c "timeout --signal=SIGINT '$time_limit'm singleMethAnalysis.sh -r '$refdir' -m '$matdir' -e '$estdir' -d '$analdir' --MC --ESI -- '$prefix' '$chr_num' '$total_proc' {X}"
...
echo "[$(date)]: Processing chromosome: ${chr_num}" 
mergeSingleMethAnalysis.sh -r "$refdir" -e "$estdir" -a "$analdir" -d "$outdir" --MC --ESI -- "$prefix" "$chr_num" "$total_proc"
