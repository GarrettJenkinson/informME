...
# Getopt command
TEMP="$(getopt -o hr:b:m:e:a:d:l: -l help,refdir:,bamdir:,matdir:,estdir:,analdir:,outdir:,ESI,MSI,MC,MATLICENSE: -n "$script_name.sh" -- "$@")"
...
SECONDS=0
echo "[$(date)]: Starting ..." 
echo "[$(date)]: Merging analysis from: ${analdir}" 
