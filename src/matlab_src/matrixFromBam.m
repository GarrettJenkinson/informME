...
% MatrixFromBAMfile(bam_prefix,chr_num,'CpGlocationPathRoot','/path/to/ref')
%
% MADATORY INPUTS:
%
% bam_prefix
...
function matrixFromBam(bam_prefix,chr_num,varargin)
...
addParameter(p,'outdir',['.' filesep 'genome' filesep],...
                @(x)validateattributes(x,{'char'},{'nonempty'}))
addParameter(p,'pairedEnds',1,@(x)validateattributes(x,{'numeric'},{'nonempty',...
               	'integer','scalar'}))         
...
addParameter(p,'regionSize',int64(3000),@(x)validateattributes(x,{'numeric'},...
		{'nonempty','integer','positive','scalar'}))     
addParameter(p,'minCpGsReqToModel',10,@(x)validateattributes(x,{'numeric'},...
		{'nonempty','integer','positive','scalar'}))           
...
matricesPathRoot    = p.Results.outdir;
...
if ~isempty(strfind(bam_prefix,'.'))
    fprintf(2,'ERROR: Input BAM file name contains period characters.\n')
    fprintf(2,'Please rename file without periods. Underscore is allowed.\n')
    return;
end
...
% Initialize. 
bamFile           = [bamFilePathRoot bam_prefix '.bam'];
...
if exist([matricesPathRoot filesep 'chr' chr_num_str filesep ...
        bam_prefix '_matrices_file_' num2str(processorNum) '.mat'],'file')
    fprintf(2,'Exiting. Final output file already exists:\n');
    fprintf(2,[matricesPathRoot filesep 'chr' chr_num_str filesep ...
        bam_prefix '_matrices_file_' num2str(processorNum) '.mat\n']);
    return;
elseif exist([matricesPathRoot filesep 'chr' chr_num_str filesep ...
              bam_prefix '_matrices.mat'],'file')
    fprintf(2,'Exiting. Final merged output file already exists:\n');
    fprintf(2,[matricesPathRoot filesep 'chr' chr_num_str filesep ...
              bam_prefix '_matrices.mat\n']);
    return;
end

% Load chromosome CpG location information.
load([CpGlocationPathRoot filesep CpGfileNameRoot chr_num_str '.mat'],'CpGlocation');
...
for startBP = thisProcessorStartBPs 
    try
        ...
        if status~=0      % Skip this region because something went wrong  
                          % with SAMtools.
            fprintf(2,['Error in SAMtools read at ' region_str '\n']);
            continue;
        else
        ...
        % Make a sparse version of the observed matrix for efficient storage
        sparseObsMatPlusOne = sparse(observedMatrix+1);

        % Write output hashtable. 
        keyStr = ['chr' chr_num_str '/bp' num2str(startBP) '-' num2str(endBP)];
        dataStruct = struct('sparseObsMatPlusOne',sparseObsMatPlusOne,...
                            'CpGlocInRegion',CpGlocInRegion);
        mapObjDataTemp(keyStr) = dataStruct;                
    
    catch exception
        fprintf(2,['Error in MatrixFromBAM in region start bp' num2str(startBP) '\n'])
        fprintf(2,getReport(exception,'extended','hyperlinks','off'));
    end
end

% Check that output directory exists, create if not.
if ~exist([matricesPathRoot filesep 'chr' chr_num_str],'dir')
    mkdir([matricesPathRoot filesep 'chr' chr_num_str]);
end

% Write output to file.
save([matricesPathRoot filesep 'chr' chr_num_str filesep bam_prefix ...
      '_matrices_file_' num2str(processorNum) '.mat'],'mapObjDataTemp','-v7.3');
