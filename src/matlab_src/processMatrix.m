...
function  [finalMatrixVec,CpGstart,CpGend] = processMatrix(dataMat)
...
if numEmptyRows>0
    ...
    fprintf(2,['WARNING: Data matrix contains ' num2str(numEmptyRows) ' empty rows\n'])
end
...
% Preallocate space. If more space is needed it is handled in a if/else
% block in the loop below.

dataMat = int32(dataMat');
finalMatrixVec = dataMat(dataMat>-1);
maxSize = length(finalMatrixVec);
CpGstart  = zeros(maxSize,1,'int32');
CpGend    = zeros(maxSize,1,'int32');
