...
matlab -nodesktop -singleCompThread -nosplash -nodisplay -c ${MATLICE} -r "try, disp('Job Running');tic;MethAnalysisForChr(${chr_num},'${phenoName}','totalProcessors',${totalProcessors},'processorNum',${processorNum},'species','${species}','ESIflag',${ESIflag},'MCflag',${MCflag});toc, catch ME, fprintf(2,'Error identifier: %s',ME.identifier),fprintf(2,'Error message: %s',ME.message), exit(1), end, exit(0);"
