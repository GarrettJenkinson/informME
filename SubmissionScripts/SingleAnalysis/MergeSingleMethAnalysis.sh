...
# Run MATLAB command
matlab -nodesktop -singleCompThread -nosplash -nodisplay -c ${MATLICE} -r "try, disp('Job Running');tic;MergeMethAnalysis(${chr_num},'${phenoName}','species','${species}','totalProcessors',${totalProcessors},'ESIflag',${ESIflag},'MCflag',${MCflag});toc, catch ME, fprintf(2,'Error identifier: %s',ME.identifier),fprintf(2,'Error message: %s',ME.message), exit(1), end, exit(0);"
