...
# Run MATLAB command
matlab -nodesktop -singleCompThread -nosplash -nodisplay -c ${MATLICE} -r "try, disp('Job Running');tic;MergeEstParams(${bamFileNames},${chr_num},'${phenoName}','species','${species}','totalProcessors',${totalProcessors});toc, catch ME, fprintf(2,'Error identifier: ',ME.identifier),fprintf(2,'Error message: ',ME.message), exit(1), end, exit(0);"
